use crate::{calculate_swap_amount_in, calculate_swap_amount_out, CurveType};
...
fn test_equivalent_to_typescript_examples() {
    ...
    let amount_to_send = BigUint::from(100_000u32); // B = 100K quote tokens to send
...
fn test_inverse_calculation() {
    ...
    let pool_base = BigUint::from(1_000_000u32); // X = 1M base tokens
    let pool_quote = BigUint::from(2_000_000u32); // Y = 2M quote tokens
    let amount_to_receive = BigUint::from(47619u32); // A = tokens to receive
    ...
    // Should be approximately 100000 (the original amount_to_send)
